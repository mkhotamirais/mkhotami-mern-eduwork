...
const getProducts = async (req, res) => {
  try {
    ...
    ok(res, 200, "getProducts", data, { count, countCriteria });
  } catch (error) {
    err(res, 400, error);
  }
};
...
const updateProduct = async (req, res) => {
  ...
  if (req.file) {
    ...
    try {
      ...
      const prevImagePath = join(rootPath, "public/images", `${match.imageName}`);
      const newImagePath = join(rootPath, "public/images", `${data.imageName}`);
...
const deleteProduct = async (req, res) => {
  try {
    ...
    const data = await Product.findById(id);
    if (!data) return err(res, 404, `data dengan id ${id} tidak ditemukan`);
    const path = join(rootPath, "public/images", `${data.imageName}`);
    if (existsSync(path)) unlinkSync(path);
